...
- 💾 **Authoritative Persistent Database**: Centralized dynamic database path resolver (`DATABASE_PATH=/data/pm.db` on Render with persistent disk), WAL mode concurrency, Foreign Key constraints, transactional `PUT /api/board` bulk synchronization, zero hardcoded seed data, and strict `.dockerignore` policies preventing stale SQLite binaries from being baked into images.
...
- **Testing**: Vitest (44 unit/component tests across 12 suites), Playwright E2E (14 tests)

### Backend
- **Framework**: FastAPI (Python 3.13), Uvicorn ASGI Server
- **Database**: SQLite3 (WAL Mode, Foreign Keys ON, Busy Timeout 5000ms, Dynamic `/data/pm.db` resolution)
...
- **Testing**: Pytest (87 tests covering production SECRET_KEY enforcement, 23-step complete lifecycle, 19-step real-user verification, transactional sync, and multi-user isolation)
